...
import Category from "@/types/RawMeatCategory";
...
const RawMeatCategoriesPage = () => {
  ...
  async function handleFormSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const creationPromise = new Promise(async (resolve, reject) => {
      ...
      const response = await fetch("/api/rawmeatcategories", {
  ...
  async function handleDeleteCategory(category: Category) {
    const deletionPromise = new Promise(async (resolve, reject) => {
      const response = await fetch(`/api/rawmeatcategories?_id=${category._id}`, {
        method: "DELETE",
      }).then((response) => response.json());
      if (response.error) {
        reject();
        ...
        <>
          ...
          <div className="mt-16 max-w-4xl mx-auto">
            <div className="flex px-2">
              <h1 className="text-2xl font-semibold italic grow text-primary">
                Raw Meats Categories
              </h1>
              <Button
                className={`text-dark hover:text-white ${
                  showAddNewBtn ? "" : "hidden"
                }`}
                color="primary"
                disabled={submitting}
            ...
            <form
              className={`mt-12 ${
                showAddNewBtn ? "hidden" : "grid grid-cols-3 gap-6"
              }`}
            ...
            >
              <div
                className={`relative ${
                  categoryImage
                    ? ""
                    : "bg-blue-100 border-dashed border-3 border-blue-500 rounded-lg flex flex-col text-center justify-center"
                } `}
              >
                <label className="cursor-pointer h-full flex flex-col justify-center">
                  {categoryImage ? (
                    ...
                    <>
                      <UploadIcon
                        className={"w-14 fill-blue-500 place-self-center"}
                      />
                      Upload Image
                    </>
                  )}
              ...
              <div className="col-span-2 flex flex-col gap-4 py-6">
                <Input
                  ...
                  label={
                    selectedCategory
                      ? `Selected Raw Meats Category: ${selectedCategory.name}`
                      : "New Raw Meats category"
                  }
                  labelPlacement="outside"
                  placeholder={
                    selectedCategory
                      ? "Update Raw Meats category name"
                      : "Enter New Raw Meats category name"
                  }
                  value={categoryName}
                ...
                <div className="flex gap-2 mt-4">
                  <Button
                    ...
                    disabled={submitting}
                  >
                    {selectedCategory ? "Save Changes" : "Add Raw Meats Category"}
                  </Button>
                  <Button
                    color="danger"
                    variant="flat"
                    fullWidth
                    className="border border-danger hover:text-white"
                    disabled={submitting}
                    onClick={() => {
                      setShowAddNewBtn(!showAddNewBtn),
                        setSelectedCategory(null),
...
export default RawMeatCategoriesPage;
