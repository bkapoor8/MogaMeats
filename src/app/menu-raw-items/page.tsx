...
import MenuItemsTable from "@/components/features/menuItems/MenuItemsRawTable"
...
const MenuItemsPage = () => {
  ...
  function handleDeleteMenuItem(menuItem: MenuItem) {
    const deletionPromise = new Promise(async (resolve, reject) => {
      const response = await fetch(`/api/menu-raw-items?_id=${menuItem._id}`, {
        method: "DELETE"
      }).then((response) => response.json());
      if (response.error) {
        reject();
        ...
        <>
          ...
          <div className="mt-16 max-w-4xl mx-auto">
            <div className="flex px-2">
              <h1 className="text-2xl font-semibold italic grow text-primary">Raw Meat Menu Items</h1>
              <Button href={"/menu-raw-items/new"} as={Link} type="button" color="primary" className="text-dark hover:text-white" endContent={<RightArrowIcon className={"w-6"} />}>Create New</Button>
            </div>
            <div className="mt-12">
              <MenuItemsTable
                menuItems={menuItems}
